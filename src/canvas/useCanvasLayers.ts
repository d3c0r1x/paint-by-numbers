...
export function useCanvasLayers(result: PipelineResult | null): CanvasLayers {
  ...
  function getFillCanvas(): HTMLCanvasElement | null {
    ...
    const pixels = buildFillPixels(
      result.labels, result.regions, result.palette, result.width, result.height,
    )
  ...
  return {
    bgRef, lineRef, highlightRef, paintRef, previewRef,
    repaintPaint, setHighlight, setFillPreview, applyFillAll,
  }
}
