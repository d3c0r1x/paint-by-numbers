...
export async function shareBlob(blob: Blob, filename: string): Promise<'shared' | 'unsupported' | 'cancelled'> {
  const file = new File([blob], filename, { type: blob.type })
  const nav = navigator as Navigator & {
    canShare?: (data: ShareData) => boolean
  }
  if (!('share' in navigator) || typeof nav.canShare !== 'function' || !nav.canShare({ files: [file] })) {
    return 'unsupported'
  }
  try {
