...
if (existsSync(demoTemplatePath)) {
  ...
  db.prepare(`INSERT INTO catalog(id,title,category,difficulty,minutes,colors,regions,tags,preview_svg,template_json,featured,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET title=excluded.title,category=excluded.category,difficulty=excluded.difficulty,minutes=excluded.minutes,colors=excluded.colors,regions=excluded.regions,tags=excluded.tags,preview_svg=excluded.preview_svg,template_json=excluded.template_json,featured=excluded.featured`).run(demoId,'Golden Portrait','Portraits','Medium',55,t.palette.length,t.regions.length,JSON.stringify(['portrait','golden','demo','girl','featured']),preview,JSON.stringify(t),1,now())
}
...
async function api(req, res, pathname) {
  ...
  if (pathname === '/api/health' && method === 'GET') return json(res, 200, { ok:true, version:'2.0.0', sqlite:true })
  ...
  if (pathname.startsWith('/api/projects/') && method === 'GET') {
    ...
    const loadedTemplate = JSON.parse(p.template_json); if (p.source_path) loadedTemplate.sourceUrl = `/uploads/${p.source_path}`; else if (loadedTemplate.catalogId) { const c = db.prepare('SELECT id FROM catalog WHERE id=?').get(loadedTemplate.catalogId); if (c) loadedTemplate.sourceUrl = loadedTemplate.catalogId === 'portrait-girl-demo' ? '/demo/portrait-girl.jpg' : loadedTemplate.sourceUrl }
