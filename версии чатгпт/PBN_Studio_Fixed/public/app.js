...
const I18N = {
  ...
  ,"Editor’s pick":"Выбор редакции","Today’s painting":"Картина дня","Featured artwork":"Избранная работа","Open painting":"Открыть картину","View result":"Посмотреть результат",
  "Finished":"Завершено","In progress":"В процессе","New painting":"Новая картина","Upload a photo":"Загрузить фото","Personal shelf":"Личная полка",
  "Your creative timeline":"Ваша творческая история","Recent activity":"Последняя активность","Timeline":"История действий","Your studio":"Ваша студия","Masterpieces":"Шедевры",
  "Your photo":"Ваше фото","Your painting":"Ваша картина","Template":"Шаблон","Reference":"Референс","Selected":"Выбрано","Share":"Поделиться",
  "Close":"Закрыть","e.g. alex.studio":"например, alex.studio","8+ characters":"8+ символов","Start your studio":"Создайте свою студию",
  "The point is not to finish quickly. The point is to enjoy every color.":"Важно не закончить быстрее. Важно наслаждаться каждым цветом.",
  "Password":"Пароль","Repeat password":"Повторите пароль","Create account":"Создать аккаунт","Sign in":"Войти","Sign out":"Выйти",
  "Building your painting":"Создаём картину","Finding the right":"Ищем подходящие","shapes & colors.":"формы и цвета.","Balancing palette":"Балансируем палитру","Building regions":"Создаём области","Refining shapes":"Уточняем формы","Smoothing contours":"Сглаживаем контуры","Placing numbers":"Расставляем номера","Finishing":"Завершаем",
  "Generated":"Создано","smart regions":"умных областей","left":"осталось","On":"Вкл","Off":"Выкл","areas remaining · autosaved":"областей осталось · сохранено",
  "Could not generate the painting:":"Не удалось создать картину:","Search paintings, animals, places…":"Поиск картин, животных и мест…","Search the gallery…":"Поиск по каталогу…",
  "Explore gallery":"Открыть каталог","Browse gallery":"Открыть каталог","Create something new →":"Создать новое →","View gallery →":"Открыть каталог →",
  "Favorites":"Избранное","finished":"завершено","minutes tracked":"минут","creative streak":"серия дней","Artist profile":"Профиль художника","Member since":"Участник с","Studio stats":"Статистика студии","Little numbers":"Немного цифр","Your promise":"Ваш принцип","Make time to make art.":"Находите время для творчества.",
  "Start painting":"Начать раскрашивание","Nothing on the easel yet":"На мольберте пока пусто","Your wall is waiting":"Ваша галерея ждёт","Make your first favorite":"Добавьте первую работу в избранное","Tap the heart on any painting in the gallery.":"Нажмите на сердечко у понравившейся картины."
}
...
function trDynamic(text){
  ...
  return String(text)
    ...
    .replace(/updated /g,'обновлено ').replace(/(^|\s)just now/g,'$1только что').replace(/(\d+)m ago/g,'$1 мин назад').replace(/(\d+)h ago/g,'$1 ч назад').replace(/(\d+)d ago/g,'$1 дн. назад')
}
function localizeDom(root=document){
  ...
  root.querySelectorAll?.('[placeholder],[title],[aria-label],[alt]').forEach(el=>{for(const a of attrs){const v=el.getAttribute(a); if(v){const t=I18N[v]||trDynamic(v); if(t!==v) el.setAttribute(a,t)}}})
}

function setLang(lang){ state.lang=lang;localStorage.setItem('pbn_lang',lang);document.documentElement.lang=lang;render() }
...
function pageEditor(){
 ...
 if(!p) return `<div class="editor-shell"><div class="empty"><h3>${trText('No painting selected')}</h3><button class="btn btn-dark" data-route="home">${trText('Back to gallery')}</button></div></div>`
 const t=p.template, pal=t.palette||[], remaining=countRemaining(p), readonly=!state.user
 return `<div class="editor"><div class="editor-top"><button class="back-btn" data-route="home" aria-label="${trText('Back to gallery')}">${icons.back}</button><div class="editor-title"><span>${p.sourceKind==='photo'?trText('Your photo'):trText('Studio collection')}</span><strong>${escapeHtml(p.title)}</strong></div><div class="editor-actions"><button class="icon-btn" data-action="undo" ${!state.activeProject.undo?.length?'disabled':''}>${icons.undo}</button><button class="icon-btn" data-action="redo" ${!state.activeProject.redo?.length?'disabled':''}>${icons.redo}</button><button class="icon-btn ${state.focus?'active':''}" data-action="toggle-focus" title="${trText('Focus mode')}">${icons.eye}</button><select id="editor-lang-select" class="lang-select"><option value="en" ${state.lang==='en'?'selected':''}>EN</option><option value="ru" ${state.lang==='ru'?'selected':''}>RU</option></select><button class="icon-btn" data-action="reference" title="${trText('Reference')}">${icons.eye}</button><button class="icon-btn" data-action="share" title="${trText('Share')}">${icons.share}</button><button class="btn btn-dark compact" data-action="export">${trText('Export')}</button></div></div><div class="editor-body ${state.focus?'focus':''}"><aside class="toolrail"><button class="tool-btn ${state.tool==='assist'?'active':''}" data-tool="assist"><span>${icons.wand}</span><label>${trText('Smart')}</label></button><button class="tool-btn ${state.tool==='brush'?'active':''}" data-tool="brush"><span>${icons.brush}</span><label>${trText('Brush')}</label></button><button class="tool-btn ${state.tool==='fill'?'active':''}" data-tool="fill"><span>${icons.fill}</span><label>${trText('Fill')}</label></button><div class="tool-spacer"></div><button class="tool-btn" data-action="fit"><span>${icons.target}</span><label>${trText('Fit')}</label></button><button class="tool-btn" data-action="focus-help"><span>?</span><label>${trText('Guide')}</label></button></aside><main class="canvas-wrap"><div class="canvas-mode-tabs"><button class="${!state.showReference?'active':''}" data-action="template">${trText('Template')}</button><button class="${state.showReference?'active':''}" data-action="reference">${trText('Reference')}</button><span>${readonly?'':('● '+trText('Auto-saved'))}</span></div><div class="canvas-stage" id="stage"><canvas id="base-canvas"></canvas><canvas id="paint-canvas"></canvas><canvas id="line-canvas"></canvas><canvas id="fx-canvas"></canvas><div class="mini-map" id="mini-map"></div></div><div class="canvas-hud"><button class="hud-btn" data-action="zoom-out">−</button><span id="zoom-label">${Math.round(state.zoom*100)}%</span><button class="hud-btn" data-action="zoom-in">＋</button><button class="hud-btn" data-action="fit">${trText('Fit')}</button></div>${readonly?`<div class="readonly-banner">🔒 ${state.lang==='ru'?'Войдите, чтобы раскрашивать и сохранять прогресс':'Sign in to paint and save progress'}</div>`:''}</main><aside class="palette-panel"><div class="palette-top"><div><span class="section-kicker">${trText('Color assistant')}</span><h2>${trText('Choose a number')}</h2></div><span class="remaining">${remaining}<small>${trText('left')}</small></span></div><div class="selected-color" id="selected-color"></div><div class="palette-list" id="palette-list">${pal.map((c,i)=>`<button class="palette-item ${state.selectedColor===i?'active':''}" data-color="${i}"><span class="swatch" style="background:${colorFrom(c.hex)}"></span><b>${symbol(i)}</b><span>${c.hex}</span><em>${regionCountForColor(t,i)}</em></button>`).join('')}</div><div class="assistant-card"><div class="assistant-icon">${icons.spark}</div><div><strong>${trText('Find all')} ${symbol(state.selectedColor)}</strong><p>${trText('Highlight every area that still needs this color.')}</p></div><button class="toggle ${state.highlight?'on':''}" data-action="highlight">${state.highlight?trText('On'):trText('Off')}</button></div><div class="progress-block"><div><span>${trText('Progress')}</span><strong>${p.progress.toFixed(0)}%</strong></div><div class="progress"><span style="width:${p.progress}%"></span></div><small>${remaining} ${trText('areas remaining · autosaved')}</small></div></aside></div></div>`
}

function renderAuth(){
 if(!state.modal) return ''
 const login=state.authMode==='login'
 return `<div class="modal-backdrop" id="auth-backdrop"><div class="auth-modal" id="auth-modal" role="dialog" aria-modal="true"><button class="modal-close" type="button" data-action="close-modal" aria-label="Close">${icons.close}</button><div class="auth-mark">PBN</div><div class="eyebrow">${trText(login?'Welcome back':'Start your studio')}</div><h2>${trText(login?'Your next painting is waiting.':'Create your artist account.')}</h2><p>${trText(login?'Save progress, keep your favorites and pick up any painting on another day.':'Register in seconds. Your paintings, progress and history stay yours.')}</p><form id="auth-form" novalidate><label>${trText('Username')}<input name="username" type="text" autocomplete="username" autocapitalize="off" spellcheck="false" required minlength="3" maxlength="24" pattern="[A-Za-z0-9_.-]+" placeholder="${trText('e.g. alex.studio')}" /></label><label>${trText('Password')}<input name="password" type="password" autocomplete="${login?'current-password':'new-password'}" required minlength="8" placeholder="${trText('8+ characters')}" /></label>${!login?`<label>${trText('Repeat password')}<input name="confirm" type="password" autocomplete="new-password" required minlength="8" placeholder="${trText('Repeat password')}" /></label>`:''}<div class="auth-error" id="auth-error" role="alert"></div><button class="btn btn-dark full-btn" type="submit">${trText(login?'Sign in':'Create account')} <span>→</span></button></form><div class="auth-switch">${trText(login?'New to Studio?':'Already have an account?')} <button type="button" data-action="toggle-auth">${trText(login?'Create account':'Sign in')}</button></div><div class="auth-note">🔒 ${trText('Passwords are stored as secure scrypt hashes. Session cookies are HttpOnly.')}</div></div></div>`
}
...
function bind(){
 ...
 $$('[data-action]', $('#app')).forEach(el=>{
  ...
  el.addEventListener('click', async e=>{
    if(el.id==='auth-backdrop') { if(e.target===el){state.modal=null;render()} return }
    if(e.target.closest?.('[data-action="toggle-favorite"]')) return
    e.preventDefault(); e.stopPropagation(); await action(el.dataset.action,el)
  })
 })
 ...
 const modal=$('#auth-modal'); if(modal) ['pointerdown','click'].forEach(ev=>modal.addEventListener(ev,e=>e.stopPropagation())); const backdrop=$('#auth-backdrop'); if(backdrop) backdrop.onclick=e=>{if(e.target===backdrop){state.modal=null;render()}}
 const lang=$('#lang-select'); if(lang) lang.onchange=e=>setLang(e.target.value)
 const editorLang=$('#editor-lang-select'); if(editorLang) editorLang.onchange=e=>setLang(e.target.value)
 const first=$('#auth-modal input'); if(first) setTimeout(()=>first.focus(),0)
}
...
async function action(a,el){
 switch(a){
  ...
  case 'template': state.showReference=false; return render()
...
function symbol(i){return String(Number(i)+1)}
...
function regionCountForColor(t,i){const filled=new Set(state.activeProject?.project?.state?.filled||[]);return (t.regions||[]).filter((r,idx)=>r.colorIdx===i&&!filled.has(idx)).length}
...
async function drawEditor(){
 ...
 function drawTemplatePaper(ctx,t,w,h){
   ...
   for(let i=0;i<ids.length;i++){const o=i*4;d[o]=255;d[o+1]=253;d[o+2]=248;d[o+3]=255}
 ...
 function paintAllDone(pc,t){const c=pc.getContext('2d');c.clearRect(0,0,w,h);const filled=new Set(state.activeProject.project.state?.filled||[]);if(!filled.size)return;const ids=t.labelsB64?base64ToTyped(t.labelsB64):null;if(!ids){for(const rid of filled){const r=t.regions[rid];if(!r)continue;c.fillStyle=t.palette[r.colorIdx]?.hex||'#888';c.fillRect(r.x,r.y,r.x2-r.x,r.y2-r.y)}return}const img=c.getImageData(0,0,w,h),d=img.data;const rgbCache=t.palette.map(x=>hexToRgb(x.hex));for(let p=0;p<ids.length;p++){const rid=ids[p];if(!filled.has(rid))continue;const rgb=rgbCache[t.regions[rid]?.colorIdx]||[136,136,136],o=p*4;d[o]=rgb[0];d[o+1]=rgb[1];d[o+2]=rgb[2];d[o+3]=255}c.putImageData(img,0,0)}
...
function paintPoint(p){const ap=state.activeProject,t=ap.project.template;if(!t||p.x<0||p.y<0)return;if(!state.user){requireAuth(()=>{});return;}const rid=pointRegion(t,p.x,p.y);if(rid<0)return;state._lastRegion=rid;const color=state.selectedColor;
...
async function init(){document.documentElement.lang=state.lang;try{const me=await api('/api/auth/me');state.user=me.user;await Promise.all([loadCatalog(),state.user?loadProjects():Promise.resolve()]);render();if(new URLSearchParams(location.search).get('demo')==='1'){const demo=state.catalog.find(x=>x.id==='portrait-girl-demo');if(demo)openCatalog(demo.id)}}catch(e){document.body.innerHTML='<div style="padding:40px;font-family:system-ui"><h2>Studio could not start</h2><p>'+escapeHtml(e.message)+'</p></div>'}}
init()
